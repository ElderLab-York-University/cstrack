...
from dataset.jde_tracker import LoadImages_jde
...
from tracker.jde_tracker import JDETracker
from tracker.cstrack_trt import CSTrack
...
def eval_seq(opt, dataloader, data_type, result_filename,seq,save_dir=None, frame_rate=30):
    if save_dir:
    ...
    for (path, img, img0, frame_id_start) in dataloader:
        ...
        online_targets = tracker.update(img, img0, seq,save_dir)
        online_tlwhs = []
        online_ids = []
        ...
        if frame_id % 20 == 0:
            ...
            logger.info(f'Person tracking at {fps:.2f} FPS')
...
def run(opt):
    ...
    result_root = os.path.join('data/results', opt.exp_name)
    ...
    for seq in opt.seqs:
        ...
        dataloader = LoadImages_jde(osp.join(opt.data_root, seq, 'img1'), opt.img_size,opt.val_hf)
        result_filename = os.path.join(result_root, '{}.txt'.format(seq))
        meta_info = open(os.path.join(opt.data_root, seq, 'seqinfo.ini')).read()
        frame_rate = int(meta_info[meta_info.find('frameRate') + 10:meta_info.find('\nseqLength')])
        nf, ta, tc, detection_num,id_num,frame_id_start = eval_seq(opt, dataloader, data_type, result_filename,seq,
                              save_dir=output_dir, frame_rate=frame_rate)
        ...
        evaluator = Evaluator(opt.data_root, seq, data_type)
    ...
    summary = Evaluator.get_summary(accs, opt.seqs, metrics)
    ...
    Evaluator.save_summary(summary, os.path.join(result_root, 'summary_{}.xlsx'.format(opt.exp_name)))
...
def main():
    ...
    parser.add_argument('--path_trt_engine',type=str, default='cfg/cstrack.trt', help='path to TensorRT engine file')
    ...
    opt.gpus = [opt.device]
    opt.data_root = '/data/MOT16/train'
    opt.exp_name = 'MOT16'
    ...
    if not opt.val_mot16:
        opt.seqs = ['Venice-2']
        opt.data_root = os.path.join(opt.data_dir, 'MOT15/images/train')
    else:
        opt.seqs = ['MOT16-02', 'MOT16-04', 'MOT16-05', 'MOT16-09', 'MOT16-10', 'MOT16-11', 'MOT16-13']
        opt.data_root = os.path.join(opt.data_dir, 'MOT16/train')

    if opt.test_mot16:
        opt.seqs = ['MOT16-01', 'MOT16-03', 'MOT16-06', 'MOT16-07', 'MOT16-08', 'MOT16-12', 'MOT16-14']
        opt.data_root = os.path.join(opt.data_dir, 'MOT16/test')

    if opt.test_mot15:
        opt.exp_name = 'MOT15'
        opt.seqs = [
            'ADL-Rundle-1', 'ADL-Rundle-3', 'AVG-TownCentre', 'ETH-Crossing', 'ETH-Jelmoli',
            'ETH-Linthescher', 'KITTI-16', 'KITTI-19', 'PETS09-S2L2', 'TUD-Crossing', 'Venice-1'
        ]
        opt.data_root = os.path.join(opt.data_dir, 'MOT15/images/test')

    if opt.test_mot17:
        opt.exp_name = 'MOT17'
        opt.seqs = ['MOT17-01-SDP', 'MOT17-03-SDP', 'MOT17-06-SDP', 'MOT17-07-SDP', 'MOT17-08-SDP', 'MOT17-12-SDP', 'MOT17-14-SDP']
        opt.data_root = os.path.join(opt.data_dir, 'MOT17/images/test')

    if opt.val_mot17:
        opt.exp_name = 'MOT17'
        opt.seqs = ['MOT17-02-SDP', 'MOT17-04-SDP', 'MOT17-05-SDP', 'MOT17-09-SDP', 'MOT17-10-SDP', 'MOT17-11-SDP', 'MOT17-13-SDP']
        opt.data_root = os.path.join(opt.data_dir, 'MOT17/images/train')

    if opt.val_mot15:
        opt.exp_name = 'MOT15'
        opt.seqs = [
            'KITTI-13', 'KITTI-17', 'ETH-Bahnhof', 'ETH-Sunnyday', 'PETS09-S2L1', 'TUD-Campus',
            'TUD-Stadtmitte', 'ADL-Rundle-6', 'ADL-Rundle-8', 'ETH-Pedcross2'
        ]
        opt.data_root = os.path.join(opt.data_dir, 'MOT15/images/train')

    if opt.val_mot20:
        opt.exp_name = 'MOT20'
        opt.seqs = ['MOT20-01', 'MOT20-02', 'MOT20-03', 'MOT20-05']
        opt.data_root = os.path.join(opt.data_dir, 'MOT20/images/train')

    if opt.test_mot20:
        opt.exp_name = 'MOT20'
        opt.seqs = ['MOT20-04', 'MOT20-06', 'MOT20-07', 'MOT20-08']
        opt.data_root = os.path.join(opt.data_dir, 'MOT20/images/test')

    opt.model = CSTrack(opt.path_trt_engine)
